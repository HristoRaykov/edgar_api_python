...
var Taxonomies = {
  ...
  updateStyleTaxonomies : function( ) {
    ...
    var foundStyles = document.getElementById('dynamic-xbrl-form').querySelectorAll('[style*="-ix-hidden"]');
    var foundStylesArray = Array.prototype.slice.call(foundStyles);
    
    foundStylesArray.forEach(function( current ) {
      
      var updatedStyle = Taxonomies.fixStyleString(current.getAttribute('style').replace("-esef-ix-hidden",
          "-sec-ix-hidden"));
    ...
    var foundRedlineStyles = document.getElementById('dynamic-xbrl-form').querySelectorAll('[style*="-ix-redline"]');
    var foundRedlineStylesArray = Array.prototype.slice.call(foundRedlineStyles);
    
    if ( HelpersUrl.getAllParams.hasOwnProperty('redline') && HelpersUrl.getAllParams.redline ) {
      
      foundRedlineStylesArray.forEach(function( current ) {
        
        var updatedStyle = Taxonomies.fixStyleString(current.getAttribute('style').replace("-esef-ix-redline",
            "-sec-ix-redline"));
  ...
  addPopover : function( element ) {
    ...
    popoverHtml += '<h3 class="reboot popover-header text-center text-popover-clamp-1 py-0"></h3>';
    popoverHtml += '<div class="reboot text-center text-popover-clamp-2 py-1">'
        + FiltersValue.getFormattedValue(element, true) + '</div>';
    popoverHtml += '<div class="reboot text-center p-2">'
        + FiltersContextref.getPeriod(element.getAttribute('contextref')) + '</div>';
    popoverHtml += '<p class="reboot text-center p-2">Click for additional information.</p>';
    popoverHtml += '</div>';
